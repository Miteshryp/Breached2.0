...
export default function LeaderboardScreen(props) {
    ...
    return (
        <div>
                ...
                <div className="w-full h-full flex my-5 justify-center items-center scrollbar-none">
                    <table className="w-full table-fixed">
                        <thead className="bg-contest-card-hover">
                            ...
                            <td className="pl-16 pr-10 py-7 text-white text-xl font-inter font-bold justify-center rounded-tr-xl ">Score</td>
                        </thead>
                        ...
                        { responseData.rankList.map((element, index) => {
                            return  (
                                <tr className={`group ${responseData.rank === index + 1 ? "bg-emerald-500" : "bg-gray-600/50"} rounded`}>
                                    <td className=" pl-4 pr-20  py-5 text-white text-xl font-inter font-thin group-last:rounded-bl-xl "> {index + 1} </td>
                                    <td className=" pl-4 pr-10  py-5 text-white text-xl font-inter font-thin"> {element.name} </td>
                                    <td className=" pl-4 pr-10  py-5 text-white text-xl font-inter font-thin"> {element.regNo} </td>
                                    <td className=" pl-16 pr-10 py-5 text-white text-xl font-inter font-thin group-last:rounded-br-xl"> {element.score} </td>    
                                </tr>
                            )
                        })
            ...
            <Modal sm visible={failShow.status && !fetching} fullScreen>
            ...
            <Modal sm fullScreen visible={fetching}>
